...
add_action('profile_update', function ($user_id, $old_user) {
    ...
    if (!empty($payload_customer_id)) {
        ...
        if ($company_name) {
            //Incase we detect company name is set use that for name also
            $arg['name'] = $company_name;
            $arg['attrs']['Billing Company'] = $company_name;
            $arg['attrs']['Contact Name'] = $user->display_name;
        }

        Payload\Customer::filter_by(array('id' => $payload_customer_id))->update(
            $arg
        );
    }
}, 10, 2);
...
add_action('woocommerce_checkout_order_processed', function ($order_id, $posted_data, $order) {
    ...
    setup_payload_api();
    $customer_pl_id = get_payload_customer_id($customer_id, $order);
    $order->set_customer_id(get_user_by_pl_id($customer_pl_id));
    $order->save();
}, 10, 3);

function get_user_by_pl_id($customer_pl_id)
{
    $users = get_users(array(
        'meta_key'   => 'payload_customer_id',
        'meta_value' => $customer_pl_id,
        'number'     => 1,
        'count_total' => false,
    ));
...
function create_guest_subscription_from_order($order)
{
...
function get_payload_customer_id($user_id = null, $order = null)
{
    $payload_customer_id = null;
    if (!empty($order)) {
        $user_id = create_guest_subscription_from_order($order);
    }
    ...
    if (!$payload_customer_id && !empty($user) && $user->user_email) {
        // Check if customer PL id is empty and if it is attempt to find by email
    ...
    if (! $payload_customer_id && !empty($user) && $user->user_email && $user->user_nicename) {
        // If customer PL id is still empty create new customer if the user Object is valid
        ...
        $customer = Payload\Customer::create($arg);
