...
?>
        ...
        <script>
            if (window.plMountPaymentMethodForm) window.plMountPaymentMethodForm()
        </script>
<?php
    }
    ...
    public function create_payment_for_order($order, $amount, $payment_method_id)
    {
        if (empty($payment_method_id)) {
            throw new TransactionDeclined('Transaction creation failed: No Payment Method assigned');
        }
        //Check the order type to ensure PL only process shop_order types
    ...
    public function create_token($payment_method, $set_current_user = null, $order = null)
    {
        ...
        // Add guard to force PL user creation on subscription transaction if non exist or is Guest
        if (!empty($order) && $order instanceof WC_Order && function_exists('wcs_cart_contains_subscription') && wcs_cart_contains_subscription() && empty(get_current_user_id())) {
            $set_current_user = get_payload_customer_id(0, $order);
        }
        if ($set_current_user) {
    ...
    public function check_if_card_exist($token)
    {
        ...
        foreach ($chk_tokens as $chk_token) {
            if (
                $chk_token->get_last4() == $token->get_last4() &&
